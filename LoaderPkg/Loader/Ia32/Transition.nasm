...
ASM_PFX(CallKernelThroughGateAsm):
    ...
    ; 1. Disable paging.
    mov eax, cr0        ; save prev value
    and eax, 0x7FFFFFFF ; 31-st bit off
...
AsmWithOurGdt:
    ...
    mov eax, cr4        ; save prev value
    or eax, 0x20         ; 5-th bit on
    ...
    rdmsr                ; read MSR[ecx] to EDX:EAX 
    or eax, 0x900        ; Low order bits
    wrmsr                ; write EDX:EAX to ECX specified MSR
